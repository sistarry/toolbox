...
case "$OS" in
    alpine)
        ...
        # 执行 Alpine 适配版脚本
        bash <(curl -sL https://raw.githubusercontent.com/sistarry/toolbox/main/Alpine/AZAPReality.sh)
        ;;
    debian|ubuntu|centos|rocky|almalinux|fedora)
        echo -e "${GREEN}检测到系统为 $OS${RESET}"
        # 执行原版脚本
        bash <(curl -sL https://raw.githubusercontent.com/sistarry/toolbox/main/PROXY/vlessreality.sh)
        ;;
    *)
