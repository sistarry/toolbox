...
menu() {
    clear
    echo -e "${GREEN}=========================${RESET}"
    echo -e "${GREEN}  ◈   PVE 管理面板   ◈  ${RESET}"
    echo -e "${GREEN}=========================${RESET}"
    echo -e "${GREEN}1) 添加SWAP${RESET}"
    echo -e "${GREEN}2) 检测环境${RESET}"
    echo -e "${GREEN}3) PVE主体安装${RESET}"
    echo -e "${GREEN}4) 预配置环境${RESET}"
    echo -e "${GREEN}5) 自动配置宿主机网关${RESET}"
    echo -e "${GREEN}6) 开设KVM小鸡${RESET}"
    echo -e "${GREEN}7) 开设LXC小鸡${RESET}"
    ...
    case $choice in
        ...
        6)
            ...
            bash <(curl -sL https://raw.githubusercontent.com/sistarry/toolbox/main/NAT/PVEKVM.sh)
            pause
            ;;
        7)
            echo -e "${GREEN}正在开设 LXC 小鸡...${RESET}"
            bash <(curl -sL https://raw.githubusercontent.com/sistarry/toolbox/main/NAT/PVELXC.sh)
            pause
            ;;
        0)
            exit 0
