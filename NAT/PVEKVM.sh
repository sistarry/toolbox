...
create_vm() {
    ...
    ./buildvm.sh $VMID $USERNAME $PASSWORD $CPU $MEM $DISK $SSHPORT ${PORT80:-0} ${PORT443:-0} $STARTPORT $ENDPORT $OS $STORAGE ${IPV6:-N}
}
...
# 删除所有虚拟机
delete_all_vms() {
    echo -e "${RED} 警告：此操作将删除所有虚拟机、清空 NAT 规则并重置网络！${RESET}"
...
menu() {
    clear
    echo -e "${GREEN}============================${RESET}"
    echo -e "${GREEN}   ◈  PVE容器 KVM 管理  ◈  ${RESET}"
    echo -e "${GREEN}============================${RESET}"
