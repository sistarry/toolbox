...
ZFS_INSTALL="https://raw.githubusercontent.com/xkatld/lxdapi-web-server/main-stable/Shell/lxdapi_zfsinstall.sh"
...
IMAGE_XX="https://raw.githubusercontent.com/sistarry/toolbox/main/NAT/LXDAPIX.sh"
...
menu() {
    ...
    echo -e "${GREEN} 2) 安装 ZFS (Ubuntu 跳过)${RESET}"
    echo -e "${GREEN} 3) 部署 LXDAPI${RESET}"
    echo -e "${GREEN} 4) 更新 LXDAPI${RESET}"
    echo -e "${GREEN} 5) 管理存储池${RESET}"
    echo -e "${GREEN} 6) 管理镜像${RESET}"
    echo -e "${GREEN} 7) 新增镜像${RESET}"
    echo -e "${GREEN} 8) 查看公网网卡${RESET}"
    echo -e "${GREEN} 9) LXDAPI面板信息${RESET}"
    ...
    case $choice in
        ...
        2) run "$ZFS_INSTALL" "安装 ZFS" ;;
        3) run "$API_INSTALL" "部署 LXDAPI" ;;
        4) run "$API_UPDATE" "更新 LXDAPI" ;;
        5) run "$STORAGE_POOL" "存储池管理" ;;
        6) run "$IMAGE_IMPORT" "镜像管理" ;;
        7) run "$IMAGE_ADD" "新增镜像" ;;
        8) show_ip ;;
        9) run "$IMAGE_XX" "面板信息" ;;
        0) exit 0 ;;
        *)
