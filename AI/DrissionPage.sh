...
check_root() {
    if [ "$(id -u)" -ne 0 ]; then
        echo -e "${RED}✘ 请使用 root 用户或 sudo 运行${RESET}"
        exit 1
    fi
}
...
uninstall_dp() {
    echo -e "${YELLOW}▶ 正在卸载 DrissionPage 及相关组件...${RESET}"
    echo -e "\n${RED}准备进入全面卸载清除流...${RESET}"
    read -r -p "确定要彻底移除 DrissionPage 及其所有浏览器残留缓存吗？(y/n): " confirm_un
