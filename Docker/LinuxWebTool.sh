#!/bin/bash
# =================================================================
# LinuxWebTool (LWT) Docker Compose 管理面板 
# =================================================================
...
get_status_info() {
    ...
    if [ "$(docker ps -aq -f name=^/${CONTAINER_NAME}$)" ]; then
        ...
        webui_port=$(docker inspect -f '{{(index (index .NetworkSettings.Ports "5270/tcp") 0).HostPort}}' "$CONTAINER_NAME" 2>/dev/null)
        [[ -z "$webui_port" ]] && webui_port="5270"
...
install_utils() {
    ...
    echo -e "\n${CYAN}====== 2. 管理员账号密码配置 ======${RESET}"
    echo -ne "${YELLOW}请输入管理员用户名 [默认: admin]: ${RESET}"
    read -r admin_user
    [[ -z "$admin_user" ]] && admin_user="admin"

    echo -ne "${YELLOW}请输入管理员密码 [默认: admin123]: ${RESET}"
    read -r admin_pass
    [[ -z "$admin_pass" ]] && admin_pass="admin123"

    echo -e "\n${CYAN}====== 3. 网络端口配置 ======${RESET}"
    echo -ne "${YELLOW}请输入 LinuxWebTool 访问端口 [默认: 5270]: ${RESET}"
    read -r custom_port
    [[ -z "$custom_port" ]] && custom_port="5270"
...
EOF
    ...
    echo -e "${YELLOW}正在通过 Docker Compose 启动 LinuxWebTool...${RESET}"
    ...
    echo -e "${GREEN}================================${RESET}"
    echo -e "${GREEN}      LinuxWebTool 部署成功！   ${RESET}"
    echo -e "${GREEN}================================${RESET}"
    echo -e "${YELLOW}访问地址      : http://${DETECT_IP}:${custom_port}${RESET}"
    echo -e "${YELLOW}初始账号      : ${admin_user}${RESET}"
    echo -e "${YELLOW}初始密码      : ${admin_pass}${RESET}"
...
update_utils() {
    ...
    echo -e "${YELLOW}正在从远端拉取 LinuxWebTool 最新镜像...${RESET}"
...
# 卸载 LinuxWebTool
uninstall_utils() {
    echo -e "${RED}警告: 卸载如果清理数据，将永久丢失您的数据库、凭据与密钥配置！${RESET}"
    echo -ne "${YELLOW}确定要卸载并删除 LinuxWebTool 容器吗？(y/n): ${RESET}"
    read -r confirm
...
menu() {
    ...
    get_status_info
    echo -e "${GREEN}================================${RESET}"
    echo -e "${GREEN}  ◈  LinuxWebTool 管理面板  ◈  ${RESET}"
    echo -e "${GREEN}================================${RESET}"
