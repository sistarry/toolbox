...
install_watchtower() {
    ...
    echo -e "${GREEN}1) 仅在【发生镜像更新/报错】时通知 (${YELLOW}推荐，防刷屏${RESET})${RESET}"
    echo -e "${GREEN}2) 每次检查完都通知 (即使没有更新也会发“一切正常”)${RESET}"
...
show_info() {
    ...
    echo -e "${YELLOW}运行模式       : 标签过滤自动更新 (仅更新带特定 label 的容器)${RESET}"
    echo -e "${YELLOW}定时检查规则   : ${custom_schedule}${RESET}"
    echo -e "${YELLOW}通知过滤策略   : $( [ "$notification_level" = "warn" ] && echo "仅更新与错误时通知" || echo "每次检查均通知" )${RESET}"
    echo -e "${GREEN}========================================${RESET}"
    echo -e "${CYAN}💡 使用提示:${RESET}"
    ...
    echo -e "${YELLOW}    labels:${RESET}"
    echo -e "${YELLOW}      - com.centurylinklabs.watchtower.enable=true${RESET}"
    echo -e "${YELLOW}注意:${GREEN}${RED}加完标签并重启该容器后，Watchtower 才能在定时时间识别并帮它更新。${GREEN}"
    echo -e "${GREEN}========================================${RESET}"
