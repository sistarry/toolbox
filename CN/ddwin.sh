...
install_v4dd() {
    ...
    bash <(curl -sSL https://v6.gh-proxy.org/https://raw.githubusercontent.com/leitbogioro/Tools/master/Linux_reinstall/InstallNET.sh) -windows 10 -lang "cn"
    show_account_info
    prompt_reboot
}
...
install_v6dd() {
    ...
    if command -v curl >/dev/null 2>&1; then
        curl -fsSL -O https://v6.gh-proxy.org/https://raw.githubusercontent.com/bin456789/reinstall/main/reinstall.sh || { echo -e "${RED}下载失败${RESET}"; return; }
    else
        wget -q https://v6.gh-proxy.org/https://raw.githubusercontent.com/bin456789/reinstall/main/reinstall.sh -O reinstall.sh || { echo -e "${RED}下载失败${RESET}"; return; }
    fi
