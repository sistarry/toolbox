    ...
    66)
        if command -v bt &>/dev/null; then
            ...
            if [ -f /usr/bin/curl ]; then
                curl -ksSO https://www.aapanel.com/script/install_panel_en.sh
            else
                wget --no-check-certificate -O install_panel_en.sh https://www.aapanel.com/script/install_panel_en.sh
            fi
